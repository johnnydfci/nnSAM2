...
class SAM2VideoPredictor(SAM2Base):
    ...
    def propagate_in_video(
        ...
        reverse=False,
        iou_score_return = False,
        multimasks_return = False,
    ):
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            ...
            ious_per_obj  = [None] * batch_size
            multimasks_per_obj = [None] * batch_size
            ...
            for obj_idx in range(batch_size):
                ...
                ious_per_obj[obj_idx] = current_out["ious"]
                multimasks_per_obj[obj_idx] = current_out["low_res_multimasks"] 

            # Resize the output mask to the original video resolution (we directly use
            # the mask scores on GPU for output to avoid any CPU conversion in between)
            
            if len(pred_masks_per_obj) > 1:
                all_ious = torch.cat(ious_per_obj, dim=0)
                all_pred_masks = torch.cat(pred_masks_per_obj, dim=0)
                all_pred_multimasks = torch.cat(multimasks_per_obj, dim=0)
            else:
                all_ious = ious_per_obj[0]
                all_pred_masks = pred_masks_per_obj[0]
                all_pred_multimasks = multimasks_per_obj[0]
            ...
            if iou_score_return and multimasks_return:
                yield frame_idx, obj_ids, video_res_masks, all_ious, all_pred_multimasks
                
            elif iou_score_return:     
                
                yield frame_idx, obj_ids, video_res_masks, all_ious
    ...
    def _run_single_frame_inference(
        ...
        pred_multimasks_gpu = current_out["low_res_multimasks"]
        ...
        compact_current_out = {
            ...
            "low_res_multimasks": pred_multimasks
        }
